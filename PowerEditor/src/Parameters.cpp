...
static bool getBoolFromXmlAttribute(const NppXml::Element& elem, const char* name)
{
...
static void setBoolToXmlAttribute(NppXml::Element& elem, const char* name, bool isYes)
{
				...
				childNode = NppXml::nextSiblingElement(childNode, "File"))
			{
				...
				if (fileName)
				{
					...
					MapPosition mapPosition{
						...
						._isWrap = getBoolFromXmlAttribute(childNode, "mapIsWrap")
					};
					...
					const bool isUserReadOnly = getBoolFromXmlAttribute(childNode, "userReadOnly");
					const bool isPinned = getBoolFromXmlAttribute(childNode, "tabPinned");
					const bool isUntitleTabRenamed = getBoolFromXmlAttribute(childNode, "untitleTabRenamed");
					...
					sfi._isRTL = getBoolFromXmlAttribute(childNode, "RTL");
	...
	if (root)
	{
		...
		for (size_t k = 0; k < nbElem ; ++k)
		{
			...
			for (const auto& vsFile : viewSessionFiles)
			{
				...
				setBoolToXmlAttribute(fileNameNode, "userReadOnly", (vsFile._isUserReadOnly && !vsFile._isMonitoring));
				...
				setBoolToXmlAttribute(fileNameNode, "RTL", vsFile._isRTL);
				setBoolToXmlAttribute(fileNameNode, "tabPinned", vsFile._isPinned);
				...
				setBoolToXmlAttribute(fileNameNode, "mapIsWrap", vsFile._mapPos._isWrap);
