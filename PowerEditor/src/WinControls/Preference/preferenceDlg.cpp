...
intptr_t CALLBACK PreferenceDlg::run_dlgProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	switch (message) 
	{
		...
		case NPPM_INTERNAL_SETTOOLICONSSET: // Set icons set only option (checkbox) on general sub-dialog, the remained real operations will be done in NppDarkMode::refreshDarkMode
		{
			...
			const int toolbarFluentColor = NppDarkMode::getToolBarFluentColorType(static_cast<bool>(wParam));
			...
			{
				const auto boundMax = static_cast<int>(NppDarkMode::FluentColorType::monochromeAccent);
				...
				nppGUI._toolBarFluentColorType = static_cast<NppDarkMode::FluentColorType>(idxFluentColor);
			}

			::SendDlgItemMessage(hGeneralSubDlg, IDC_COMBO_TOOLBAR_ICON_COLOR, CB_SETCURSEL, static_cast<WPARAM>(nppGUI._toolBarFluentColorType), 0);
			...
			::EnableWindow(::GetDlgItem(hGeneralSubDlg, IDC_COMBO_TOOLBAR_ICON_COLOR), (nppGUI._toolBarStatus != TB_STANDARD) ? TRUE : FALSE);
...
intptr_t CALLBACK GeneralSubDlg::run_dlgProc(UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	switch (message)
	{
		case WM_INITDIALOG:
		{
			...
			const int fluentColorType = static_cast<int>(nppGUI._toolBarFluentColorType);
			...
			addComboItem(IDC_COMBO_TOOLBAR_ICON_COLOR, L"Standard");
			addComboItem(IDC_COMBO_TOOLBAR_ICON_COLOR, L"Accent");
			addComboItem(IDC_COMBO_TOOLBAR_ICON_COLOR, L"Monochrome");
			addComboItem(IDC_COMBO_TOOLBAR_ICON_COLOR, L"Monochrome accent");

			::SendDlgItemMessage(_hSelf, IDC_COMBO_TOOLBAR_ICON_COLOR, CB_SETCURSEL, fluentColorType, 0);

			::EnableWindow(::GetDlgItem(_hSelf, IDC_COMBO_TOOLBAR_ICON_COLOR), (tbStatus != TB_STANDARD) ? TRUE : FALSE);
		...
		case WM_CTLCOLORSTATIC:
		{
			...
			{
				return NppDarkMode::onCtlColorDarkerBGStaticText(reinterpret_cast<HDC>(wParam), (::IsWindowEnabled(::GetDlgItem(_hSelf, IDC_COMBO_TOOLBAR_ICON_COLOR))));
			}
			return NppDarkMode::onCtlColorDarker(reinterpret_cast<HDC>(wParam));
		}
						...
						case CBN_SELCHANGE : // == case LBN_SELCHANGE :
						{
							switch (LOWORD(wParam))
							{
								...
								case IDC_COMBO_TOOLBAR_ICON:
								{
									...
									::EnableWindow(::GetDlgItem(_hSelf, IDC_COMBO_TOOLBAR_ICON_COLOR), (idxIconSet != TB_STANDARD) ? TRUE : FALSE);
								...
								case IDC_COMBO_TOOLBAR_ICON_COLOR:
								{
									...
									auto idxFluentColor = static_cast<int>(::SendDlgItemMessage(_hSelf, IDC_COMBO_TOOLBAR_ICON_COLOR, CB_GETCURSEL, 0, 0));
									idxFluentColor = std::min<int>(idxFluentColor, boundMax);
									NppDarkMode::setToolBarFluentColorType(idxFluentColor, NppDarkMode::isEnabled());
									return TRUE;
								}
